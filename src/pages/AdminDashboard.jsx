...
function SessionPanel({ admin }) {
  ...
  const fetchSessions = async () => {
    ...
    try {
      ...
      setActiveSession(active || null);
      if (active) {
        // Use _id (session_instance_id) not session_id — so morning/afternoon
        // runs of the same session name are counted separately
        const resp = await getSessionResponses(active._id);
        setResponses(resp);
      } else {
        setResponses([]);
      }
